"""chroma_agent — Exa-flavored port of Chroma Context-1's search/grep/prune harness.

Two-stage pipeline:
1. `ChromaHarness` runs the multi-turn loop with chunked search + grep + prune,
   rewriting past tool_result history in place so pruned URLs disappear from the
   context.
2. `Extractor` (the same BrowseComp-shaped extractor lean_searcher uses) turns
   the surviving live chunks into Exact Answer / Explanation / Confidence.
"""
...
class ChromaAgent(BaseAgent):
    def __init__(self, config_path: Path, model_configs: dict[str, dict]):
        ...
        if "searcher" not in model_configs:
            raise ValueError(
                "chroma_agent requires a searcher model. "
                "Pass --model <name> or --searcher-model <name>."
            )
        if "extractor" not in model_configs:
            raise ValueError(
                "chroma_agent requires an extractor model. "
        ...
        self.harness = ChromaHarness(
            ...
            num_results_per_search=budget.get("num_results_per_search", 5),
            num_sentences_per_highlight=budget.get("num_sentences_per_highlight", 3),
            highlights_per_url=budget.get("highlights_per_url", 5),
